...
public class Welch {
    ...
    public boolean isEqualMu(){
        return tTest.tTest(valuesA, valuesB, alpha);
    }

    //TODO: Allow checks with significance level, i.e. A-B < 10
    public double getT(){
        return round(tTest.t(valuesA, valuesB));
    }

    public double getWelchDF(){
 ...
 */
    ...
    /* OLD: Wrong formula, possibly pooled? 
    public double getWelchDF(){
        /* Formula: https://online.stat.psu.edu/stat415/lesson/3/3.2
         * (sdA^2/nA + sdB^2/nB)^2
         * /
        ...
        double numerator = Math.pow(sdA,2)/nA + Math.pow(sdB,2)/nB;
        numerator = Math.pow(numerator,2);
        double denomA = Math.pow((Math.pow(sdA,2)/nA),2);
        denomA = denomA/(nA-1.0);
        double denomB = Math.pow((Math.pow(sdB,2)/nB),2);
        denomB = denomB/(nB-1.0);
        double finalNum;
        try {
            System.out.println("DenomA: " + denomA);
                System.out.println("\tnA -1: " + (nA-1));
 ...
 */
    public double[] getWelchCI(){
        /* Formula: https://online.stat.psu.edu/stat415/lesson/3/3.2
         * avgA - avgB +- t(alpha,df) * sqrt(sdA^2/nA + sdB^2/nB)
         * 
         */
        double[] range = new double[2];
        double t = StatisticalSummary.getTStar(alpha, getWelchDF());
        ...
        range[0] = round(xbarDiff - tPart);
        range[1] = round(xbarDiff + tPart);

        return range;
    }
    
    public double[] getPooledCI(){
        /* Formula: https://online.stat.psu.edu/stat415/lesson/3/3.1
         * (avgA - avgB) +- t(alpha) Sp sqrt(1/nA + 1/nB)
         * Sp = sqrt(((nA - 1)sdA^2 + (nB-1)sdB^2)/(nA+nB-2))
        */
        double[] ci = new double[2];
        double means = (meanA - meanB);
        double multiplier = StatisticalSummary.getTStar(alpha, (nA + nB - 2.0));
        double sds = Math.sqrt(
            (
                ((nA-1)*Math.pow(sdA,2))
                +((nB-1)*Math.pow(sdB,2))
            )
        ...
        double sqrt = Math.sqrt(1.0/nA + 1.0/nB);
        ci[0] = means - multiplier*sds*sqrt;
        ci[1] = means + multiplier*sds*sqrt;
        return ci;
    }
    ...
    public String printTestWordy() {
        ...
        //boolean tTestResult; 
        double tStat; 
        double criticalVal = StatisticalSummary.getTStar(alpha, 64);
        ...
        result += ". |";
        result += tStat;
        result +="| compared to a critical value of ";
        result += -1.0; //TODO: Add critical value call from statistical summary
        result += " (t* where alpha is ";
        result += alpha;
        result += " and df is ";
        result += df;
        result += ") finds that "; 
